...
def test_mean_and_low_rank_cov_eq_mean_and_low_rank_cov(X, rank, tree,
                                                        reset_func):
    ...
    X_list = list(X)
    if reset_func:
        def reset(x): return iter(X_list)
    ...
    cov1 = np.reshape(
        cov1,
        [cov1.shape[0], np.prod(cov1.shape[1:])]
    )
    cov2 = np.reshape(
        cov2,
        [cov2.shape[0], np.prod(cov2.shape[1:])]
    )
...
def test_autocorrelation_step_eq_no_step(X, rank, tree):
    ...
    corr1 = np.reshape(
        corr1,
        [corr1.shape[0], np.prod(corr1.shape[1:])]
    )
    corr2 = np.reshape(
        corr2,
        [corr2.shape[0], np.prod(corr2.shape[1:])]
    )
    # scipy svd used for the low rank approximations is non deterministic
    # and even the same call twice will yield different results
    # errors depend on the conditioning of the matrices
